...
def match_distances_with_rsrp_forward_fill_area(df, time_limit_ms=300, area=None):
    """
    Simple and efficient version using forward fill for RSRP values with time limit
    ...
    # Forward fill the RSRP columns with time limit
    print("Forward filling RSRP values with time limit...")
    df_sorted['LTE KPI PCell Serving RSRP [dBm]'] = forward_fill_with_time_limit(
        df_sorted, 'LTE KPI PCell Serving RSRP [dBm]', time_limit_ms
    )
    df_sorted['5G KPI PCell RF Serving SS-RSRP [dBm]'] = forward_fill_with_time_limit(
        df_sorted, '5G KPI PCell RF Serving SS-RSRP [dBm]', time_limit_ms
    )
    ...
    if area is not None:
        df_sorted = df_sorted[df_sorted['area'] == area].copy()
        if len(df_sorted) == 0:
            return {}
...
if 1:
    ...
    mainland_ta_dist_dict_by_area = {area: {} for area in area_types}
    mainland_rsrp_distance_dict_by_area = {area: {} for area in area_types}
    ...
    for op in op_list:
        for day in ['day_1', 'day_2', 'day_3', 'day_4']:
            ...
            merged_df = pd.concat([df_template, timing_advance_df]).reset_index(drop=True)
            merged_df = merged_df.sort_values("Timestamp").reset_index(drop=True)
            df_template = merged_df.copy()

            # Check if 'area' column exists
            if 'area' not in df_template.columns:
                print(f"Warning: 'area' column not found in {op}_{day}")
                continue
            ...
            for area in area_types:
                ...
                area_distance_rsrp_pair, area_df = match_distances_with_rsrp_forward_fill_area(df_template, area=area)
                ...
                area_ta_data = mainland_ta_dist_dict_by_area[area]
                area_rsrp_data = mainland_rsrp_distance_dict_by_area[area]
                ...
                area_ta_data[0][op].extend(lte_rach_area)  # LTE RACH distances
                area_ta_data[1][op].extend(lte_non_rach_area)  # LTE non-RACH distances
                
                # LTE RSRP data
                area_rsrp_data[0][op].extend([pair for pair in area_distance_rsrp_pair['lte_rach_distance_rsrp']])  # LTE RACH (distance, RSRP) pairs
                area_rsrp_data[1][op].extend([pair for pair in area_distance_rsrp_pair['lte_distance_rsrp']])  # LTE non-RACH (distance, RSRP) pairs
                ...
                if not missing_columns:
                    ...
                    area_ta_data[2][op].extend(fiveg_rach_area)  # 5G RACH distances
                    area_ta_data[3][op].extend(fiveg_non_rach_area)  # 5G non-RACH distances
                    
                    # 5G RSRP data
                    area_rsrp_data[2][op].extend([pair for pair in area_distance_rsrp_pair['fiveg_rach_distance_rsrp']])  # 5G RACH (distance, RSRP) pairs
                    area_rsrp_data[3][op].extend([pair for pair in area_distance_rsrp_pair['fiveg_distance_rsrp']])  # 5G non-RACH (distance, RSRP) pairs

            print(f"Completed processing {op}_{day}")
    ...
    # Save a combined dictionary for all areas
    combined_mainland_ta_dict = {
        'urban': mainland_ta_dist_dict_by_area['urban'],
        'suburban': mainland_ta_dist_dict_by_area['suburban'],
        'rural': mainland_ta_dist_dict_by_area['rural']
    }
    
    combined_mainland_rsrp_dict = {
        'urban': mainland_rsrp_distance_dict_by_area['urban'],
        'suburban': mainland_rsrp_distance_dict_by_area['suburban'],
        'rural': mainland_rsrp_distance_dict_by_area['rural']
    }
    ...
    print("\nMainland Summary by area type:")
    for area in area_types:
        print(f"\n{area.upper()} AREA:")
        ta_data = mainland_ta_dist_dict_by_area[area]
        rsrp_data = mainland_rsrp_distance_dict_by_area[area]
        
        for op in op_list:
            lte_rach_count = len(ta_data[0][op])
            lte_non_rach_count = len(ta_data[1][op])
            fiveg_rach_count = len(ta_data[2][op])
            fiveg_non_rach_count = len(ta_data[3][op])
            
            lte_rach_rsrp_count = len(rsrp_data[0][op])
            lte_non_rach_rsrp_count = len(rsrp_data[1][op])
            fiveg_rach_rsrp_count = len(rsrp_data[2][op])
            fiveg_non_rach_rsrp_count = len(rsrp_data[3][op])
            
            print(f"  {op}: LTE_RACH({lte_rach_count}/{lte_rach_rsrp_count}), LTE_NonRACH({lte_non_rach_count}/{lte_non_rach_rsrp_count}), 5G_RACH({fiveg_rach_count}/{fiveg_rach_rsrp_count}), 5G_NonRACH({fiveg_non_rach_count}/{fiveg_non_rach_rsrp_count})")
