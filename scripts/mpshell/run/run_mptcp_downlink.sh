    ...
    for SUBFOLDER in "$LOCATION_TRACE_DIR"/*; do
        if [[ -d "$SUBFOLDER" ]]; then
            ...
            while read -r TRACE_ID TRACE_DURATION; do
		        ...
		        ## **Run Test**
		        bash -c "
		            echo 'Running cellular test $TRACE_ID for $OPERATOR_1 on RTT $CELL_DELAY_1 and $OPERATOR_2 on RTT $CELL_DELAY_2'
		            mpshell '$CELL_DELAY_1' '$CELL_FILE_1' '$TEST_UP_FILE' '$CELL_DELAY_2' '$CELL_FILE_2' '$TEST_UP_FILE' /usr/local/bin/iperf3 -m -s & 
		            sleep 1
		            iperf3 -m -c 100.64.0.2 -i 0.5 -t '$TRACE_DURATION' -R > '$TRACE_OUTPUT_DIR/mptcp_down_default.txt'
		            sleep 2
		            sudo pkill -f iperf3
