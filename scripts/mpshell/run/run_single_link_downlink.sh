...
trace_dir_for_location() {
    case "$1" in
        alaska) echo "$DATASETS_DIR/alaska_road_test_202406/emulation/formated_traces/downlink" ;;
        hawaii) echo "$DATASETS_DIR/hawaii_road_test_202408/emulation/formated_traces/downlink" ;;
        *) return 1 ;;
    esac
}
    ...
    for SUBFOLDER in "$LOCATION_TRACE_DIR"/*; do
        if [[ -d "$SUBFOLDER" ]]; then
            ...
            while read -r TRACE_ID TRACE_DURATION; do
                ...
                for TRACE_FILE in "$SUBFOLDER/$TRACE_ID"*; do
                    if [[ -f "$TRACE_FILE" ]]; then
                        ...
                        if (( CELL_COUNT == 0 )); then
                            ...
                            echo "Found trace: $CELL_FILE_1 with operator $OPERATOR_1 and RTT $CELL_DELAY_1"
                        elif (( CELL_COUNT == 1 )); then
                            CELL_FILE_2="$TRACE_FILE"
                            OPERATOR_2="$OP"
                            CELL_DELAY_2="${DELAY_MAP[$OPERATOR_2]}"
                            CELL_COUNT=$((CELL_COUNT + 1))
                            echo "Found trace: $CELL_FILE_2 with operator $OPERATOR_2 and RTT $CELL_DELAY_2"
                        fi
                    fi
                done
                ...
                if [[ $CELL_SIZE_1 -gt 100 ]]; then
                    TRACE_OUTPUT_DIR="$OUTPUT_DIR/$LOCATION/$SUBFOLDER_NAME/$TRACE_ID"
                    mkdir -p "$TRACE_OUTPUT_DIR"

                    ## Cellular test for operator 1
                    bash -c "
                        echo 'Running cellular test for $OPERATOR_1 on $TRACE_ID...'
                        mpshell '$CELL_DELAY_1' '$CELL_FILE_1' '$TEST_UP_FILE' '$CELL_DELAY_1' '$CELL_FILE_1' '$TEST_UP_FILE' /usr/local/bin/iperf3 -s & 
                        sleep 1
                        iperf3 -c 100.64.0.2 -i 0.5 -t '$TRACE_DURATION' -R > '$TRACE_OUTPUT_DIR/single_down_${OPERATOR_1}.txt'
                        sleep 2
                        sudo pkill -f iperf3
            ...
            if [[ $CELL_SIZE_2 -gt 100 ]]; then
                    TRACE_OUTPUT_DIR="$OUTPUT_DIR/$LOCATION/$SUBFOLDER_NAME/$TRACE_ID"
                    mkdir -p "$TRACE_OUTPUT_DIR"
                    ## Cellular test for operator 2
                    bash -c "
                        echo 'Running cellular test for $OPERATOR_2 on $TRACE_ID...'
                        mpshell '$CELL_DELAY_2' '$CELL_FILE_2' '$TEST_UP_FILE' '$CELL_DELAY_2' '$CELL_FILE_2' '$TEST_UP_FILE' /usr/local/bin/iperf3 -s & 
                        sleep 1
                        iperf3 -c 100.64.0.4 -i 0.5 -t '$TRACE_DURATION' -R > '$TRACE_OUTPUT_DIR/single_down_${OPERATOR_2}.txt'
                        sleep 2
                        sudo pkill -f iperf3
                        sudo pkill -f mpshell
                        wait
