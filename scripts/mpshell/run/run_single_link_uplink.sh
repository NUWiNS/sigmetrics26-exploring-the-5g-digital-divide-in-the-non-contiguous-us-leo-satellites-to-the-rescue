        ...
        if [[ -d "$SUBFOLDER" ]]; then
            ...
            while read -r TRACE_ID TRACE_DURATION; do
                ...
                if [[ $CELL_SIZE_1 -gt 100 ]]; then
                    ...
                    bash -c "
                        ...
                        mpshell '$CELL_DELAY_1' '$TEST_UP_FILE' '$CELL_FILE_1' '$CELL_DELAY_2' '$TEST_UP_FILE' '$CELL_FILE_2' /usr/bin/stdbuf -oL iperf3 -s -i 0.5 | tee '$TRACE_OUTPUT_DIR/single_up_${OPERATOR_1}.txt' & 
                        sleep 1
                        iperf3 -c 100.64.0.2 -i 0.5 -t '$TRACE_DURATION' > /dev/null
                        sleep 2
                        sudo pkill -f iperf3
            ...
            if [[ $CELL_SIZE_2 -gt 100 ]]; then
                    ...
                    bash -c "
                        ...
                        mpshell '$CELL_DELAY_1' '$TEST_UP_FILE' '$CELL_FILE_1' '$CELL_DELAY_2' '$TEST_UP_FILE' '$CELL_FILE_2' /usr/bin/stdbuf -oL iperf3 -s -i 0.5 | tee '$TRACE_OUTPUT_DIR/single_up_${OPERATOR_2}.txt' & 
                        sleep 1
                        iperf3 -c 100.64.0.4 -i 0.5 -t '$TRACE_DURATION' > /dev/null
                        sleep 2
                        sudo pkill -f iperf3
                        sudo pkill -f mpshell
                        wait
