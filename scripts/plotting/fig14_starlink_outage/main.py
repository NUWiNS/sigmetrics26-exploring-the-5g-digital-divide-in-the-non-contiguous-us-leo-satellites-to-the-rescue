...
from scripts.configs.alaska import ROOT_DIR as ALASKA_ROOT_DIR
from scripts.configs.hawaii import ROOT_DIR as HAWAII_ROOT_DIR
from scripts.configs.mainland import ROOT_DIR as LA_TO_OMAHA_ROOT_DIR

current_dir = os.path.dirname(os.path.abspath(__file__))
output_dir = os.path.join(current_dir, "outputs")

def get_starlink_metrics(root_dir: str, suffix: str = ''):
    if suffix:
        filename = f"starlink_metric.app_tput_filtered.{suffix}.csv"
    else:
        filename = "starlink_metric.app_tput_filtered.csv"

    metric_csv = os.path.join(
        root_dir, "starlink", filename
    )
...
def plot_outage_rate_by_area(
    ...
    ax.set_ylim(0, 12.5)
    ax.set_yticks(np.arange(0, 13, 2.5))
    ax.grid(True)
    # Set x-axis ticks and labels
    ax.set_xticks(
        x_positions,
        [location_conf[loc]["abbr"] for loc in sorted(plot_data.keys(), key=lambda x: location_conf[x]["order"])]
    )
    
    # Add legend
    ax.legend(loc='upper left', prop={'size': 10, 'weight': 'bold'})
    
    # Adjust layout and save
    plt.tight_layout()
    output_path = os.path.join(output_dir, "outage_rate_by_area.pdf")
...
def plot_outage_reason_distribution_by_area_with_white_list(
    ...
    # Adjust layout and save
    plt.tight_layout()
    output_path = os.path.join(output_dir, "outage_cause_distribution_filtered.la_to_omaha.pdf")
...
def main():
    ...
    alaska_df = get_starlink_metrics(os.path.join(ALASKA_ROOT_DIR, 'processed'))
    hawaii_df = get_starlink_metrics(os.path.join(HAWAII_ROOT_DIR, 'processed'))
    # maine_df = get_starlink_metrics(MAINE_ROOT_DIR)
    la_to_omaha_df = get_starlink_metrics(os.path.join(LA_TO_OMAHA_ROOT_DIR, 'processed'), suffix='2024-11-01-2024-11-05')
    ...
    plot_data = {
        ...
        # },
        "la_to_omaha": {
            "urban": filter_data_by_area(la_to_omaha_df, "urban"),
            "rural": filter_data_by_area(la_to_omaha_df, "rural"),
        },
    }
